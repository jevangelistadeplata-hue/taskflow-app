...
<body class="bg-slate-100 text-slate-800 p-8">
    <div class="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <h1 class="text-2xl font-bold text-slate-900 mb-6">Editar Tarea</h1>
        ...
        <form action="{{ route('tareas.update', $tarea) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="descripcion" id="descripcion" rows="3"
                    class="w-full mt-1 p-2 border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ old('descripcion', $tarea->descripcion) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="date" name="fecha_limite" id="fecha_limite" value="{{ old('fecha_limite', $tarea->fecha_limite) }}"
                    class="w-full mt-1 p-2 border border-slate-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none">
            </div>

            <div class="flex justify-end space-x-2 pt-4">
                <a href="{{ route('tareas.index') }}" class="bg-slate-300 hover:bg-slate-400 text-slate-800 px-4 py-2 rounded-md">Cancelar</a>
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2 rounded-md">Actualizar Tarea</button>
            </div>
        </form>
    </div>
</body>
